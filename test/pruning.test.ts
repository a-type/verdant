...
it('prunes invalid data in entities with changes from outdated clients', async () => {
	...
	// this one won't be recoverable at all
	item2.set('yikes', 'foo');
	...
	await waitForQueryResult(clientB2.items.get('2'), (v) => !v);
	...
	await waitForEntitySnapshot(getItem1.current, {
		id: '1',
		contents: 'hello',
