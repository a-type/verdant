...
import { authorization, Entity } from '@verdant-web/store';
...
it('applies correct authz to a migrated document which has changed its authz since creation', async () => {
	const {
		server,
		createGenericClient: createClient,
		library,
	} = createTestContext({
		library: 'authz-migration-changed',
	});
	...
	const item: Entity = await client.items.findOne().resolved;
	expect(item).not.toBeNull();
	...
	const recreated = await client.items.put(
		{
			id: 'item-1',
			contents: 'recreated',
		},
	...
	expect(recreated.access).toBe(authorization.public);
	...
	expect(item2.access).toBe(authorization.public);
});
